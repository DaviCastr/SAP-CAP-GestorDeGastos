sap.ui.define([
    ...
    return {
        onUploadImage: async function () {
            const oFileUploader = this.byId("FileUploadDetailsCartao");
            const oPath = this._view.getBindingContext().getPath();
            const oCartao = this._view.getBindingContext().getObject();
            ...
            var oETagHeader = new sap.ui.unified.FileUploaderParameter({
                name: "If-Match",
                value: oCartao["@odata.etag"] || "*" // Use "*" para ignorar a validação de ETag (se permitido)
            });
        ...
        onUploadCompleto: async function (oEvent) {
            ...
            try {
                ...
                const aFiles = oFileUploader.oFileUpload.files;
                const oFile = aFiles[0];
                const oAvatar = this.byId("avatarImagemCartao");

                var oProgressBar = this.byId("uploadImagemCartao");
